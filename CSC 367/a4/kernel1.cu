                 ...
                 int32_t *output, int32_t width, int32_t height, float * transfer_in_time, float *computation_time ,float *transfer_out_time) 
{
  ...
  //Calculate required size for each device variable and array. 
  ...
  //transfer in event
  //input_device and filter_device array will contain value from input and filter.
  ...
  //computation event 
  //Computation step is divided into 2 steps, first step including calling kernel to process pixel, 
  //second step include finding global max and min and normalizing.
  //Because there is a memcpy operation in between these two steps,
  //so calculate two different time and add them at the end
  ...
  kernel1<<<num_block, num_of_thread_per_block>>>(filter_device, dimension, input_device, output_device, width, height, max_array_device, min_array_device);
  ...
  cudaStatus = cudaMemcpy(max_array_cpu, max_array_device, SIZE_max_min_array, cudaMemcpyDeviceToHost);
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpy fail: %s\n", cudaGetErrorString(cudaStatus));
  }
  cudaStatus = cudaMemcpy(min_array_cpu, min_array_device, SIZE_max_min_array, cudaMemcpyDeviceToHost);
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpy fail: %s\n", cudaGetErrorString(cudaStatus));
  }
  ...
  normalize1<<<num_block, num_of_thread_per_block>>>(output_device, width, height, global_max_cpu, global_min_cpu);
  cudaDeviceSynchronize();
                        ...
                        int32_t height, int *max_array, int *min_array) 
{
  ...
  //Each block has 1024 threads. Each thread store a max and min value in these two array respectively.
  //Hence, each array must have enough space on shared memory 
  ...
  //Notice this kernel is column major
  int row_ind, col_ind;
  col_ind = global_tid/height;
  row_ind = global_tid%height;
  
  //These syncthreads ensure synchronization
  if(global_tid < width * height) 
  {
    output[row_ind * width + col_ind] = apply2d(filter, dimension, input, row_ind, col_ind, width, height);
  }
  __syncthreads();

  local_max_array[local_tid] = output[row_ind * width + col_ind];
  local_min_array[local_tid] = output[row_ind * width + col_ind];
  __syncthreads();
...
/* normalize1 kernel normalize every pixel stored in output. Access pattern is column major*/
