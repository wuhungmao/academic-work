...
def parse_perf(x):
  ...
  fp_regex = ('.*\s((\d+\.\d*)|(\d+))\s*{}\s*(#.*L1-dcache accesses)?\s*' + 
...
default_file = '1'
def run_perf(filter, method, numthreads = 8, chunk_size = 2, input_file=default_file, repeat =
    10):
...
def graph(mode, filter = "3x3"):
  ...
  for method in methods:
    for nthread in threads:
      if method == "work queue":
        if nthread <4:
          local_results[method] += [4]
          continue
      chunk_size = nthread
  ...
  if mode == 'l1d_loadmisses':
    title = "L1 cache misses with work queue"
  else:
    title = "execution time with work queue"
  ylabel = mode
  if mode == 'time':
    ylabel += "(s)"
  ...
  plotter.graph(
    ...
    'graph_{}.png'.format(mode+" "+ filter+ " with work queue"), # filename
    title,
    "# Threads", # xlabel
    ylabel
  )
