...
void run_kernel3(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  //Calculate total number pixel assigned to a thread. 
  ...
  kernel3<<<num_block, num_of_thread_per_block>>>(filter_device, dimension, input_device, output_device, width, height, max_array_device, min_array_device, chunk_per_thread);
  ...
  normalize3<<<num_block, num_of_thread_per_block>>>(output_device, width, height, global_max_cpu, global_min_cpu, chunk_per_thread);
  cudaDeviceSynchronize();
                        ...
                        int32_t height, int *max_array, int *min_array, int chunk_per_thread) 
{
  ...
  //The idea is different from kernel 1 and 2. In kernel 3 and 4, 
  //every thread will first find local max and local min within the assigned
  //chunk. And these will be stored in two arrays. 
