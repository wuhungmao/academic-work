...
{
	// clear_log();
	printf("Testing syscall %d flag 1\n", syscall);
	do_intercept(syscall, 0);
	printf("Testing syscall %d flag 2\n", syscall);
	do_intercept(syscall, -EBUSY);
	printf("Testing syscall %d flag 3\n", syscall);
	do_as_guest("./test_intercept nonroot %d", syscall, 0);
	printf("Testing syscall %d flag 4\n", syscall);
	do_release(syscall, 0);
	printf("Testing syscall %d flag 5\n", syscall);
}
...
int main(int argc, char **argv)
{
	...
	{
		printf("Non-root user test for syscall %s\n", argv[2]);
