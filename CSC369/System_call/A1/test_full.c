...
int vsyscall_arg(int sno, int n, ...)
{
	...
	va_end(va);
	// printf("\nCalling syscall %d with cmd %ld, syscall %ld, pid %ld\n", sno, args[0], args[1], args[2]);
...
{
	// EPERM: a process attempts to do something that requires higher privileges
	printf("Non-root user test\n");
	printf("do intercept\n");
...
void test_syscall(int syscall)
{
	...
	// EBUSY: a device or resource is currently in use by another process
	do_intercept(syscall, -EBUSY);
	do_as_guest("./test_full nonroot %d", syscall, 0);
	// EINVAL: an invalid argument was supplied
	printf("Entering do_start\n");
...
int main(int argc, char **argv)
{
	...
	// run insmod interceptor.ko as shell command
	...
	// test_syscall(SYS_clock_gettime);
