...
typedef struct
{
	...
	int lstcount;
	struct list_head monitored_pids_lst;
} sys_call_metadata_t;

/* An entry for each system call in this "metadata" table */
sys_call_metadata_t sys_call_metadata_table[NR_syscalls];

/* Access to the system call table and your metadata table must be synchronized */
spinlock_t sys_call_metadata_table_lock = SPIN_LOCK_UNLOCKED;
...
static int add_pid_sysc(pid_t pid, int sysc)
{
	...
	INIT_LIST_HEAD(&ple->lst_curr_pos);
	ple->pid = pid;

	list_add(&ple->lst_curr_pos, &(sys_call_metadata_table[sysc].monitored_pids_lst));
	sys_call_metadata_table[sysc].lstcount++;
...
static int del_pid_sysc(pid_t pid, int sysc)
{
	...
	list_for_each(i, &(sys_call_metadata_table[sysc].monitored_pids_lst))
	{
		ple = list_entry(i, struct pid_list, lst_curr_pos);
		if (ple->pid == pid)
		{
			...
			if (sys_call_metadata_table[sysc].lstcount == 0 && sys_call_metadata_table[sysc].monitored == 1)
			{
				sys_call_metadata_table[sysc].monitored = 0;
			}
...
static int del_pid(pid_t pid)
{
	...
	{
		list_for_each_safe(i, n, &(sys_call_metadata_table[s].monitored_pids_lst))
		{
			...
			if (ple->pid == pid)
			{
				...
				sys_call_metadata_table[s].lstcount--;
				/* If there are no more pids in sysc's list of pids, then
				 * stop the monitoring only if it's not for all pids (monitored=2) */
				if (sys_call_metadata_table[s].lstcount == 0 && sys_call_metadata_table[s].monitored == 1)
				{
					sys_call_metadata_table[s].monitored = 0;
				}
			}
		}
	}
...
static void destroy_list(int sysc)
{
	...
	list_for_each_safe(i, n, &(sys_call_metadata_table[sysc].monitored_pids_lst))
	{
		ple = list_entry(i, struct pid_list, lst_curr_pos);
		list_del(i);
		kfree(ple);
	...
	sys_call_metadata_table[sysc].lstcount = 0;
	sys_call_metadata_table[sysc].monitored = 0;
}
...
{
	spin_lock(&sys_call_metadata_table_lock);
	del_pid(current->pid);
	spin_unlock(&sys_call_metadata_table_lock);
 ...
 *     monitored=1 => some pids are monitored, check the corresponding monitored_pids_lst
...
{
	long result;
	if ((sys_call_metadata_table[reg.orig_ax].monitored == 1 || sys_call_metadata_table[reg.orig_ax].monitored == 2) && check_pid_monitored(reg.orig_ax, current->pid) == 1)
	{
		log_message(current->pid, reg.orig_ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}

	// call original system call
	result = (sys_call_metadata_table[reg.orig_ax].f)(reg);
	return result;
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid)
{
	...
	case REQUEST_SYSCALL_INTERCEPT:
		if (current_uid() == 0)
		{
			...
			{
				if (sys_call_metadata_table[syscall].intercepted == 0)
				{
					// intercept syscall
					spin_lock(&sys_call_table_lock);
					sys_call_metadata_table[syscall].f = sys_call_table[syscall];
					set_addr_rw((unsigned long)sys_call_table);
					sys_call_table[syscall] = interceptor;
					set_addr_ro((unsigned long)sys_call_table);
					spin_unlock(&sys_call_table_lock);
					sys_call_metadata_table[syscall].intercepted = 1;
					return_value = 0;
				}
				else
				{
					// cannot intercept a syscall that is already intercepted
					return_value = -EBUSY;
				}
			}
			else
	...
	case REQUEST_SYSCALL_RELEASE:
		if (current_uid() == 0)
		{
			if (pid_task(find_vpid(pid), PIDTYPE_PID))
			{
				if (sys_call_metadata_table[syscall].intercepted == 1)
				{
					// de-intercept syscall
					sys_call_metadata_table[syscall].intercepted = 0;
					sys_call_metadata_table[syscall].monitored = 0;
					// clear monitored pids list
					destroy_list(syscall);
					// restore original syscall in syscall table
					spin_lock(&sys_call_table_lock);
					set_addr_rw((unsigned long)sys_call_table);
					sys_call_table[syscall] = sys_call_metadata_table[syscall].f;
					set_addr_ro((unsigned long)sys_call_table);
					spin_unlock(&sys_call_table_lock);
					return_value = 0;
				}
				...
				{
					// cannot de-intercept a system call that has not been intercepted
					return_value = -EINVAL;
				}
			}
			else
	...
	case REQUEST_START_MONITORING:
		// root owns this process
		if (current_uid() == 0)
		{
			// check validity of pid
			if (pid_task(find_vpid(pid), PIDTYPE_PID))
			{
				// check if pid is already being monitored and syscall is intercepted
				if (sys_call_metadata_table[syscall].intercepted == 1 && sys_call_metadata_table[syscall].monitored != 2 && check_pid_monitored(syscall, pid) == 0)
				{
					if (add_pid_sysc(pid, syscall) != -ENOMEM)
					{
						// set monitored flag to 1
						sys_call_metadata_table[syscall].monitored = 1;
						return_value = 0;
					}
					else
					{
				...
				{
					// pid is already being monitored -> -EBUSY, syscall not intercepted -> -EINVAL
					return_value = sys_call_metadata_table[syscall].intercepted == 0 ? -EINVAL : -EBUSY;
				}
			}
			else if (pid == 0)
			{
				if (sys_call_metadata_table[syscall].intercepted == 1)
				{
					// monitor all processes belong to caller
					sys_call_metadata_table[syscall].monitored = 2;
					return_value = 0;
				}
				else
				{
					// syscall not intercepted -> -EINVAL
					return_value = -EINVAL;
				}
			}
			else
	...
	case REQUEST_STOP_MONITORING:
		if (current_uid() == 0)
		{
			...
			if (pid_task(find_vpid(pid), PIDTYPE_PID))
			{
				if (sys_call_metadata_table[syscall].intercepted == 1 && del_pid_sysc(pid, syscall) != -EINVAL)
				{
					return_value = 0;
				}
				else
				{
					// cannot stop monitoring for a pid that is not being monitored, or if the syscall has not been intercepted yet
					return_value = -EINVAL;
				}
			}
			else if (pid == 0)
...
static int init_function(void)
{
	...
	// set table as writable
	set_addr_rw((unsigned long)sys_call_table);

	// store original MY_CUSTOM_SYSCALL function to restore it when exiting module
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];

	// replace the old system call function at MY_CUSTOM_SYSCALL
	...
	{
		sys_call_metadata_table[i].f = NULL;
		sys_call_metadata_table[i].intercepted = 0;
		sys_call_metadata_table[i].monitored = 0;
		sys_call_metadata_table[i].lstcount = 0;
		INIT_LIST_HEAD(&sys_call_metadata_table[i].monitored_pids_lst);
	}

	// initialize syscall_table_t entries
	sys_call_metadata_table[MY_CUSTOM_SYSCALL].intercepted = 1;
	sys_call_metadata_table[MY_CUSTOM_SYSCALL].monitored = 0;
	sys_call_metadata_table[MY_CUSTOM_SYSCALL].f = sys_call_table[MY_CUSTOM_SYSCALL];

	// initialize the pid list for this system call
	INIT_LIST_HEAD(&sys_call_metadata_table[MY_CUSTOM_SYSCALL].monitored_pids_lst);
	sys_call_metadata_table[MY_CUSTOM_SYSCALL].lstcount = 0;

	// set the back to read-only
	set_addr_ro((unsigned long)sys_call_table);
	spin_unlock(&sys_call_table_lock);
	spin_unlock(&sys_call_metadata_table_lock);
...
static void exit_function(void)
{
	...
	for (i = 1; i < NR_syscalls; i++)
	{
		if (sys_call_metadata_table[i].intercepted == 1)
		{
			// clear every node in the lst, set lstcount and monitored to 0
			sys_call_table[i] = sys_call_metadata_table[i].f;
			destroy_list(i);
			sys_call_metadata_table[i].intercepted = 0;
			sys_call_metadata_table[i].monitored = 0;
			sys_call_metadata_table[i].f = NULL;
