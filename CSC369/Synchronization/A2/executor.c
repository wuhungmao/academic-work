...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < NUM_QUEUES; i++) {
        pthread_create(&execution_threads[i], NULL, &execute_jobs, (void *) &tassadar.admission_queues[i]);
        pthread_create(&admission_threads[i], NULL, &admit_jobs, (void *) &tassadar.admission_queues[i]);
    }
