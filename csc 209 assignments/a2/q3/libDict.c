...
#define ROW_INIT_ENTRIES 16
#define ROW_GROW_FACTOR 2 

#define PRIME1 77933 // a large prime
#define PRIME2 119557 // a large prime

/*Find position with binary search algorithm*/
int find_new_position_recursive(DictEntry *entry, int index1, int index2, int num, char* insert_key, void * insert_value, int);

int find_new_position_linear(DictEntry *entry, int index1, int index2, char* insert_key, void *insert_value, int);

/*Find position with binary search algorithm*/
int find_existing_position_recursive(DictEntry *entry, int index1, int index2, int num, char* search_key);

int find_existing_position_linear(DictEntry *entry, int index1, int index2, int num, char* search_key);
...
DictEntry *dictGet(Dict *d, char *key){
	...
	// find key in row
	int place_of_key = find_existing_position_recursive((d->rows+h) ->entries, 0, (d->rows+h) ->numEntries-1, (d->rows+h) ->numEntries, key);
	switch (place_of_key)
	{
	case -2:
		return NULL;
	...
	case -3:
		...
		break;
	default:
		if (strcmp((((d->rows) + h) ->entries + place_of_key) ->key, key) == 0) {
		return (((d->rows) + h) ->entries + place_of_key);
		}
		break;
	}
	return NULL;
...
int dictDel(Dict *d, char *key){
	...
	int place_to_delete = find_existing_position_recursive(((d->rows) + h) -> entries, 0, ((d -> rows) + h) -> numEntries-1, ((d -> rows) + h) -> numEntries, key);
	// find key in row
	switch (place_to_delete)
	{
	case -2:
		printf("wrong index2 and index1");
		break;
	case -3:
		printf("didn't find the word in between index1 and index2");
		break;
	default:
		if (strcmp((((d->rows) + h) ->entries + place_to_delete) ->key, key) == 0) {
			// free key
			if (((d->rows) + h) -> numEntries == 1) {
				free((((d->rows) + h) ->entries + place_to_delete) ->key);
			}
		} else {
			printf("wrong place_to_delete");
		}
		break;
	}
	for (int i = place_to_delete; i<((d->rows) + h)->numEntries-1; i++) {
...
int find_existing_position_recursive(DictEntry *entry, int index1, int index2, int num, char* search_key) {
	if ((index2 - index1) <= 4){
		int result = find_existing_position_linear(entry, index1, index2, num, search_key);
		return result;
	}
	char *key = (entry + ((int) floor(num/2)+index1)) ->key;
	if (strcmp(search_key, key) > 0){
		index1 = index1+floor(num/2);
		num = floor(num/2);
		return find_existing_position_recursive(entry, index1, index2, num, search_key);
	} else if (strcmp(search_key, key) == 0) {
		return ((int) floor(num/2)+index1);
	} else if (strcmp(search_key, key) < 0) {
		index2 = index2-floor(num/2);
		num = floor(num/2);
		return find_existing_position_recursive(entry, index1, index2, num, search_key);
	}
}
...
int find_existing_position_linear(DictEntry *entry, int index1, int index2, int num, char* search_key){
	...
	for (int i = index1; i<=index2; i++) {
		...
		int result1 = strcmp(search_key, key);
		if (result1 == 0) {
			return i;
		}
	}
...
int find_new_position_recursive(DictEntry *entry, int index1, int index2, int num, char* insert_key, void * insert_value, int max) {
	if ((index2 - index1) <= 4){
		int result = find_new_position_linear(entry, index1, index2, insert_key, insert_value, max);
		return result;
	}
	char *key = (entry + ((int) floor(num/2)+index1)) ->key;
	if (strcmp(insert_key, key) > 0){
		index1 = index1+floor(num/2);
		num = floor(num/2);
		return find_new_position_recursive(entry, index1, index2, num, insert_key, insert_value, max);
	} else if (strcmp(insert_key, key) == 0) {
		(entry +  ((int) floor(num/2)+index1))-> value = insert_value;
		return -1;
	} else if (strcmp(insert_key, key) < 0) {
		index2 = index2-floor(num/2);
		num = floor(num/2);
...
int find_new_position_linear(DictEntry *entry, int index1, int index2, char* insert_key, void *insert_value, int max) {
	/*-2 for having wrong index, -1 for finding same key, -3 for not finding appropriate position -> wrong indexs*/
	if (index2 - index1 < 0) {
		return -2;
	}
	for (int i = index1; i<=index2; i++) {
		char *key = (entry+i) -> key;
		char *key2 = (entry+i+1) -> key;
		int result1 = strcmp(insert_key, key);
		if (result1 < 0) {
			return i;
		} else if (result1 == 0) {
			(entry+i) -> value = insert_value;
			return -1;
		} else if(i == max-1) {
			return i+1;
		}
		int result2 = strcmp(insert_key, key2);
...
int dictPut(Dict *d, char *key, void *value){
	...
	if (((d -> rows) + h) ->numEntries == 0) {
		place_for_new = 0;
		free((((d -> rows) + h) -> entries+place_for_new) -> key);
		...
		place_for_new = find_new_position_recursive(((d -> rows) + h) -> entries, 0,  ((d -> rows) + h) ->numEntries-1, ((d -> rows) + h) ->numEntries, key, value, ((d -> rows) + h) ->numEntries);
	}
	if (place_for_new == -1) {
		return 1;
	}
	/*DictEntry *temp = ((d -> rows) + h) -> entries + place_for_new;
	(((d -> rows) + h) -> entries+place_for_new) -> key = strdup(key);
	(((d -> rows) + h) -> entries+place_for_new) -> value = value;
	for (int i = place_for_new+1; i< ((d ->rows) + h) ->numEntries+1; i++) {
		swap(((d -> rows) + h) -> entries+i, temp);
	}*/
	int start = ((d -> rows) + h) ->numEntries;

	char *char1 = (((d -> rows) + h) -> entries+start) -> key;

	for (int start = ((d -> rows) + h) ->numEntries; start > place_for_new; start--) {
		char *char1 = (((d -> rows) + h) -> entries+start) -> key;
		free((((d -> rows) + h) -> entries+start) -> key);
		/*free((((d -> rows) + h) -> entries+start) -> value);*/
		(((d -> rows) + h) -> entries+start) -> key = strdup((((d -> rows) + h) -> entries+start-1) -> key);
		(((d -> rows) + h) -> entries+start) -> value = (((d -> rows) + h) -> entries+start-1) -> value;
		char *char2 = (((d -> rows) + h) -> entries+start-1) -> key;
		char *char3 = (((d -> rows) + h) -> entries+start) -> key; 
	}
	/*free(((d -> rows) + h) -> entries+place_for_new);*/
	(((d -> rows) + h) -> entries+place_for_new) -> key = strdup(key);
	(((d -> rows) + h) -> entries+place_for_new) -> value = value;
	((d ->rows) + h) ->numEntries++;
	#ifdef DEBUG
