...
from contract import Contract, TERM_DEPOSIT
...
class PhoneLine:
    ...
    - the <bills> dictionary contains only those month+year combinations as keys
    ...
    def new_month(self, month: int, year: int) -> None:
        ...
        if (month, year) not in self.bills:
            ...
            if len(self.bills) == 1 and self.contract.contract_type == \
                    'TermContract':
                self.bills[(month, year)].fixed_cost = TERM_DEPOSIT
            if len(self.bills) == 1 and self.contract.contract_type == \
                    'PrepaidContract':
                self.bills[(month, year)].fixed_cost = - self.contract.balance
    ...
    def make_call(self, call: Call) -> None:
        ...
        elif (call.time.month, call.time.year) in self.bills:
            self.contract.bill_call(call)
