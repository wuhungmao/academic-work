...
from contract import PrepaidContract, TermContract, MTMContract
...
def create_customers(log: dict[str, list[dict]]) -> list[Customer]:
    ...
    for cust in log['customers']:
        ...
        for line in cust['lines']:
            ...
            # 3) Do not change anything in the code below besides uncommenting i
...
def process_event_history(log: dict[str, list[dict]],
    ...
    for calls in log['events']:
        if calls['type'] == 'call':
            year, month, date = calls['time'][0:10].split('-')
            datetime_object = datetime.datetime(int(year), int(month),
                                                int(date))
            the_call = Call(calls['src_number'], calls['dst_number'],
                            datetime_object, calls['duration'],
                            calls['src_loc'], calls['dst_loc'])
            the_source_call_customer = find_customer_by_number(
                the_call.src_number, customer_list)
            the_destination_call_customer = find_customer_by_number(
                the_call.dst_number, customer_list)
            the_source_call_customer.make_call(the_call)
            the_destination_call_customer.receive_call(the_call)

    # To advance a month for all customers
        billing_date = datetime.datetime.strptime(calls['time'],
                                                  "%Y-%m-%d %H:%M:%S")
        billing_month = billing_date.month
        billing_year = billing_date.year
        if (billing_month, billing_year) not in billing_month_list:
            new_month(customer_list, billing_date.month, billing_date.year)
            billing_month_list.append((billing_month, billing_year))
